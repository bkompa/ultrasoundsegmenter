...
def dice(y_true, y_pred):
    ...
    for i in range(0,len(y_true)):
        if y_true[i].sum() == 0 and y_pred[i].sum() == 0:
            ...
            dice_score += (2. * intersection) / (y_true_f.sum() + y_pred_f.sum())
    return dice_score/len(y_true)
...
def load_subject_ids():
    subject_ids = np.load('/home/beam/projects/kaggle/ultrasound-nerve-segmentation/data/subject_ids.npy')
    return subject_ids
